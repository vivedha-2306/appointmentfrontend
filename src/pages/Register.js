...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        // Show the exact message from backend (e.g. "This email is already registered...")
  ...
  return (
    <div style={{ maxWidth: '420px', margin: '80px auto', padding: '40px 24px', background: 'white', borderRadius: '12px', boxShadow: '0 4px 20px rgba(0,0,0,0.1)' }}>
      <h1 style={{ textAlign: 'center', marginBottom: '32px', color: '#111' }}>Register</h1>

      {error && (
        <div style={{ color: '#d32f2f', marginBottom: '16px', textAlign: 'center', padding: '10px', background: '#ffebee', borderRadius: '6px' }}>
          {error}
        </div>
      )}
      ...
      <p style={{ textAlign: 'center', marginTop: '24px', color: '#555' }}>
        Already have an account? <Link to="/login" style={{ color: '#1976d2', fontWeight: 500 }}>Login</Link>
      </p>
    </div>
  );
}
